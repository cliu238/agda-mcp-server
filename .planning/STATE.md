...
- Context (motivating experiment): Loop ② is grounded in the real **agda-unimath Hopf/π₃(S²)** autoformalization with Codex→MCP (`ref/README.md`, local/untracked). Observed Codex behavior — scope-narrowing, shortcuts, flag-planting — is the empirical seed for ORCL-02/03; the **join associativity** case study (codex interrupted twice → human rocq-hott pivot) is the canonical example and a Phase-3 regression / Phase-5 fuel candidate. agda-unimath (the Hopf work) = the PROC-02 fuel corpus.
