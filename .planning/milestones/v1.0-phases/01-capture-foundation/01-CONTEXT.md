...
- **D-08 [DEFERRED — Phase 3 / empirical] [informational]:** Trimming to a **minimal single-file repro is not a Phase-1 decision**. It is empirical (needs real captured defects as material) and is explicitly Phase 3 (REPRO-01, assisted/manual minimization); automatic ddmin minimization is v2 (AUTO-01). **No spike now** — there is no captured material to trim until Phase 5 dogfooding produces it.
