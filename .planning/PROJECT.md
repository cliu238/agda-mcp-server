...
- **Upstream compatibility (fork policy)**: This repo is a fork of `InvariantHoldings/agda-mcp-server` and must stay cleanly mergeable (v1.2 Upstream Reconcile depends on it). **Do not modify upstream-origin files** (anything that also exists upstream — most of `src/`) unless genuinely necessary; prefer fork-only surfaces (`scripts/`, `.planning/`, `.agents/skills/`, fork-authored `docs/`). A necessary upstream-file change must be kept minimal and **recorded with rationale in `.planning/UPSTREAM-DEVIATIONS.md`** as a candidate upstream PR, so divergence and merge-conflict surface stay bounded.
...
| Fork policy: minimize edits to upstream-origin files; record necessary ones for a later upstream PR | As a fork that must stay mergeable with `InvariantHoldings/agda-mcp-server`, every fork-only edit to an upstream file becomes permanent merge-conflict surface; routing changes to fork-only surfaces (`scripts/`, `.planning/`, `.agents/skills/`) and upstreaming the rest keeps divergence bounded | Standing (from 2026-07-05, owner instruction) — running log in `.planning/UPSTREAM-DEVIATIONS.md`; also in root `CLAUDE.md` Constraints |
