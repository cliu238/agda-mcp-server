...
| T-05-02-05 | Tampering | `promoteCapture`'s dedup-index write, triggered per detected capture | accept | Originally `accept` — reused the existing, already-reviewed `scripts/promote-capture.mjs` unchanged (no new write path introduced by 05-02). Superseded by this consolidation: sibling plan 09-01 (same phase, DEBT-02) deletes `scripts/promote-capture.mjs` and its dedup-index write outright this same wave, so the component this threat concerned no longer exists in the codebase. | closed (component removed 2026-07, DEBT-02) |
