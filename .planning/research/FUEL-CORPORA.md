...
**Scope note:** These are the real dogfooding corpora and field-evidence artifacts for the **whole milestone — not Phase-1-only**. Every phase's researcher/planner should consult the relevant rows. Both repos are **private / access-gated** (viewable via `gh` by the maintainer's team). The `agda-mcp-ux-report/` measurements are against **agda-mcp-server v0.6.7**, which is the **current released version** (`package.json` 0.6.7 = npm 0.6.7 = latest tag `v0.6.7`), NOT an old one. `main` is +25 commits past the tag, but only **one** touches `src/` (`e38f90a`, the #65/#66 goal-ID / load-freshness fix); everything else is planning docs. So the measured defects are almost certainly **still live** — except #65/#66, which has an as-yet-unreleased src fix. "Re-verify against `main`" therefore means *confirm reproduction* (likely positive), not *check whether it was already fixed*; the flagship #64/#61 scope-check-only false-green is untouched by that commit. This inventory is the concrete backing for **PROC-02** (pinned fuel-pointer set + machine-readable policy).
